...
void main_main()
{
    ...
    for (int ilev = 0; ilev <= fine_level; ++ilev)
    {
        ...
        if (ilev < fine_level)
        {
            ...
            const iMultiFab mask = makeFineMask(
                pf.boxArray(ilev), pf.DistributionMap(ilev),
            ...
            electronsum += ParReduce(
                TypeList<ReduceOpSum>{}, TypeList<Real>{}, mfe,
                [=] AMREX_GPU_DEVICE(
                    int bno, int i, int j, int k) -> GpuTuple<Real> {
                    return (
                        (ima[bno](i, j, k) == 0) ? mae[bno](i, j, k) * vol
                                                 : 0._rt);
                });
            ...
            potsum += ParReduce(
                ...
                [=] AMREX_GPU_DEVICE(
                    int bno, int i, int j, int k) -> GpuTuple<Real> {
                    return (
                        (ima[bno](i, j, k) == 0) ? maphi[bno](i, j, k) * vol
                                                 : 0._rt);
                });

            errorsum_e += ParReduce(
                TypeList<ReduceOpSum>{}, TypeList<Real>{}, mfe,
                [=] AMREX_GPU_DEVICE(
                    int bno, int i, int j, int k) -> GpuTuple<Real> {
                    IntVect iv(i, j, k);
                    ...
                    return (
                        (ima[bno](i, j, k) == 0)
                            ? std::pow(mae[bno](i, j, k) - exactsoln, 2.0) * vol
                            : 0._rt);
                });

            errorsum_pot += ParReduce(
                TypeList<ReduceOpSum>{}, TypeList<Real>{}, mfe,
                [=] AMREX_GPU_DEVICE(
                    int bno, int i, int j, int k) -> GpuTuple<Real> {
                    IntVect iv(i, j, k);
                    ...
                    return (
                        (ima[bno](i, j, k) == 0)
                            ? std::pow(maphi[bno](i, j, k) - exactsoln, 2.0) *
                                  vol
                            : 0._rt);
                });
        } else
        {
            ...
            electronsum += ParReduce(
                ...
                [=] AMREX_GPU_DEVICE(int bno, int i, int j, int k)
                    -> GpuTuple<Real> { return (mae[bno](i, j, k) * vol); });

            potsum += ParReduce(
                TypeList<ReduceOpSum>{}, TypeList<Real>{}, mfe,
                [=] AMREX_GPU_DEVICE(int bno, int i, int j, int k)
                    -> GpuTuple<Real> { return (maphi[bno](i, j, k) * vol); });
            ...
            errorsum_pot += ParReduce(
                ...
                [=] AMREX_GPU_DEVICE(
                    ...
                    Real exactsoln = (std::pow(x, 5.0) - x) / 40.0;
                    return (
                        std::pow(maphi[bno](i, j, k) - exactsoln, 2.0) * vol);
                });
        }
    }
    ...
    amrex::Real errfrac_pot =
        amrex::Math::abs(errnorm_pot) / amrex::Math::abs(potavg);

    amrex::Print() << "L2 error average of E, pot  = " << errnorm_e << '\t'
                   << errnorm_pot << "\n";
    ...
    if (errfrac_e > 1e-3 || errfrac_pot > 1e-3)
    {
        amrex::Abort("Too much error in MMS2");
    } else
    {
        amrex::Print()
            << "MMS error less than 1e-3, electron density/potential error="
            << errfrac_e << "\t" << errfrac_pot << "\n";
