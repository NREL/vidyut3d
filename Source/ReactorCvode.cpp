  ...
  int surf_flag)
{
  ...
  // vector a_y.
  if(surf_flag){
      flag = CVodeInit(a_cvode_mem, cF_SurfRHS, a_time, a_y);
  } else {
...
)
{
  ...
  initCvode(y, A, udata, NLS, LS, cvode_mem, stream, time_start, ncells, 0);
  ...
  initCvode(y, A, udata, NLS, LS, cvode_mem, time_start, ncells, 0);
...
)
{
  ...
  // Fill data
  surf_flatten(
    box, ncells, nspec_in, yvec_d);

#ifdef AMREX_USE_OMP
  amrex::Gpu::Device::streamSynchronize();
#endif

  initCvode(y, A, udata, NLS, LS, cvode_mem, stream, time_start, ncells, 1);
  ...
  long int* d_nfe = v_nfe.data();
  surf_unflatten(
    box, ncells, nspec_in, FC_in, yvec_d, d_nfe, dt_react);
  ...
  initCvode(y, A, udata, NLS, LS, cvode_mem, time_start, ncells, 1);
  ...
  ParallelFor(
    box, [=, &CvodeActual_time_final] AMREX_GPU_DEVICE(
        ...
        amrex::Real* yvec_d = N_VGetArrayPointer(y);
        utils::surf_box_flatten<Ordering>(
          icell, i, j, k, ncells, nspec_in, yvec_d);
        ...
        utils::surf_box_unflatten<Ordering>(
          icell, i, j, k, ncells, nspec_in, yvec_d, dt_react);
  ...
  sunrealtype t, N_Vector y_in, N_Vector ydot_in, void* user_data)
{
  ...
  amrex::Real Tgas_in = udata->Tgas_vect;
  amrex::ParallelFor(ncells, [=] AMREX_GPU_DEVICE(int icell) noexcept {
    utils::fKernelSurfSpec<Ordering>(
      icell, ncells, dt_save, Tgas_in, yvec_d, ydot_d);
  });
  amrex::Gpu::Device::streamSynchronize();
  return 0;
}
